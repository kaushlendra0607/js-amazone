        ...
        {/* List Products */}
        {
          list.map((item, index) => (
            <div className='grid grid-cols-[1fr_3fr_1fr] md:grid-cols-[1fr_3fr_1fr_1fr_1fr]
             gap-2 px-2 py-1 border border-solid border-gray-300 text-sm
              text-center items-center' key={index}>
              <img className='w-20' src={item.image[0]} alt="productImage" />
              <p>{item.name}</p>
              <p>{item.category}</p>
              <p>{currency}{item.price}</p>
              <p className='text-right md:text-center cursor-pointer text-lg text-rose-400'
