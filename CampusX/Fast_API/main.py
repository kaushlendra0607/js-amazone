...
class Patient(BaseModel):
    ...
    gender: Annotated[
        Literal["male", "female", "others", "Male", "Female", "Others"],
...
class UpdatePatient(BaseModel):
    ...
    city: Annotated[Optional[str], Field(default=None, description="City living in.")]
    age: Annotated[
        Optional[int], Field(default=None, gt=0, lt=200, description="Age of patient.")
    ]
    gender: Annotated[
        Optional[Literal["male", "female", "others", "Male", "Female", "Others"]],
        Field(default=None, description="Gender of patient"),
    ]
    ht: Annotated[
        Optional[float], Field(default=None, gt=0, description="Hieght in metres")
    ]
    wt: Annotated[
        Optional[float], Field(default=None, gt=0, description="Weight in kg")
    ]
...
def sort_patients(
    ...
    if sort_by not in valid_fields:
        raise HTTPException(
            status_code=404, detail=f"Invalid field chosen. Select from {valid_fields}"
        )
...
async def create_patient(patient: Patient):
    ...
    return JSONResponse(
        content={"message": "Patient created successfully."}, status_code=200
    )
...
async def update(patient_id: str, update_patient: UpdatePatient):
    ...
    if patient_id not in data:
        raise HTTPException(
            status_code=400, detail="Patient doesnt exist. Create first."
        )
